...
export default function Accordion(){
    ...
    return(
        ...
        <Panel title="Etymology" isActive={activeIndex === 1} onShow={() => setActiveIndex(1)}>
            ...
            sering diterjemahkan sebagai "penuh dengan apel". Sebenarnya, di wilayah sekitar Almaty dipercaya sebagai asal usul apel, dan <i lang="la">Malus sieversii</i> liar dianggap sebagai kandidat 
            yang mungkin menjadi nenek moyang apel domastik modern.
        </Panel>
        </>
    );
}
...
function Panel({title, children, isActive, onShow}){
    return(
        <section className="panel border border-gray-700 p-2">
            <h3>{title}</h3>
            {isActive ? (<p>{children}</p>): (<button className="bg-blue-400 text-white p-1 rounded m-2" onClick={onShow}>Tampilkan</button>)}
        </section>
    );
}